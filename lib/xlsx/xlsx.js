...
function xlsx(file, options) { 
	...
	if (typeof file === 'string') {
		...
		if (s) {
			...
			while(--i)
			{
				// substring on </si> is unnecessary - since our nested incrementation/aggregation over <t>[text]</t> will stop at the last </t>
				//sharedStrings[i - 1] = s[i].substring(0, s[i].indexOf('</si>'));
				sharedStrings[i - 1] = s[i];
				if (sharedStrings[i - 1]){
					var stringElements = sharedStrings[i - 1].split(/<t.*?>/g);
					var joinedString = "";
					for(var stringIndex = 0;stringIndex < stringElements.length;++stringIndex)
					{
						joinedString += unescapeXML(stringElements[stringIndex].substring(0, stringElements[stringIndex].indexOf('</t>')));
					}
					sharedStrings[i - 1] = joinedString;
				}
			} // Do not process i === 0, because s[0] is the text before first t element
		}

		// Get file info from "docProps/core.xml"
		var props = zip.file('docProps/core.xml');
		s = props != null ? props.asText() : '';
    ...
    fonts = new Array(1);
		...
		zipTime = Date.now();
		var compression_method = options.compression_method || 'DEFLATE';
		result = {
			base64: zip.generate({ compression: compression_method }), zipTime: Date.now() - zipTime, processTime: processTime,
