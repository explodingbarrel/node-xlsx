...
function xlsx(file) { 
	...
	function findSheetPath(sheetId, wb, rel) {
		var key, start, end, rid, target;

		//sheetId -> r:id
		key = 'sheetId="' + sheetId + '"';

		start = wb.indexOf('r:id="', wb.indexOf(key)) + 6;
		end = wb.indexOf('"', start);
		rid = wb.substring(start, end);

		//r:id -> Target
		key = 'Relationship Id="' + rid + '"';
		start = rel.indexOf('Target="', rel.indexOf(key)) + 8;
		end = rel.indexOf('"', start);
		target = rel.substring(start, end);
   ...
   if (typeof file === 'string') { 
	   ...
	   wbxml = s = zip.file('xl/workbook.xml').asText(); index = s.indexOf('activeTab="');
	   relxml = zip.file('xl/_rels/workbook.xml.rels').asText();
		if (index > 0) {
			s = s.substr(index + 11); 
		...
		while (i--) {
			s = zip.file(findSheetPath(i+1, wbxml, relxml)).asText().split('<row ');
